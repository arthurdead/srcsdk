...
#ifdef GAME_DLL
	int				AddString( const char *value, int length = -1, const void *userdata = 0 )
	{ return AddString(true, value, length, userdata); }
#elif defined CLIENT_DLL
	int				AddString( const char *value, int length = -1, const void *userdata = 0 )
	{ return AddString(false, value, length, userdata); }
#endif
