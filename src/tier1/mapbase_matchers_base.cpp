...
ConVar mapbase_wildcards_enabled("wildcards_enabled", "1", FCVAR_NONE, "Toggles '?' wildcard and true '*' features. Useful for maps that have '?' in their targetnames.");
ConVar mapbase_wildcards_lazy_hack("wildcards_lazy_hack", "1", FCVAR_NONE, "Toggles a hack which prevents lazy '?' wildcards from picking up \"???\", the default instance parameter.");
ConVar mapbase_regex_enabled("regex_enabled", "1", FCVAR_NONE, "Toggles regex matching handover.");
