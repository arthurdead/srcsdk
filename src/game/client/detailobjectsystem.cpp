...
class CDetailModel : public IDetailModel
{
...
public:
	...
	virtual ICollideable*		GetCollideable()		{ return NULL; }		// Static props DO implement this.
...
public:
	...
	IDetailModel* GetDetailModel( int idx );
	...
	void RenderFastTranslucentDetailObjectsInLeaf( CFastDetailLeafSpriteList *pData, const DistanceFadeInfo_t &info, const Vector &viewOrigin, const Vector &viewForward, const Vector &viewRight, const Vector &viewUp, int nLeaf, Vector *pVecClosestPoint, bool bFirstLeaf );
...
private:
	...
	int SortSpritesBackToFront( int nLeaf, const Vector &viewOrigin, const Vector &viewForward, const DistanceFadeInfo_t &fadeInfo, SortInfo_t *pSortInfo );
...
RenderableTranslucencyType_t CDetailModel::ComputeTranslucencyType()
{
	if((m_Alpha < 255) || modelinfo->IsTranslucent(m_pModel))
...
// Gets a particular detail object
//-----------------------------------------------------------------------------
IDetailModel* CDetailObjectSystem::GetDetailModel( int idx )
{
...
int CDetailObjectSystem::SortSpritesBackToFront( int nLeaf, const Vector &viewOrigin, const Vector &viewForward, const DistanceFadeInfo_t &fadeInfo, SortInfo_t *pSortInfo )
{
...
void CDetailObjectSystem::RenderTranslucentDetailObjects( const DistanceFadeInfo_t &info, const Vector &viewOrigin, const Vector &viewForward, const Vector &viewRight, const Vector &viewUp, int nLeafCount, LeafIndex_t *pLeafList )
{
	...
	for ( int i = 0; i < nLeafCount; ++i )
	{
		...
		int nCount = SortSpritesBackToFront( nLeaf, viewOrigin, viewForward, info, pSortInfo );
...
void CDetailObjectSystem::RenderFastTranslucentDetailObjectsInLeaf( CFastDetailLeafSpriteList *pData, const DistanceFadeInfo_t &info, const Vector &viewOrigin, const Vector &viewForward, const Vector &viewRight, const Vector &viewUp, int nLeaf, Vector *pVecClosestPoint, bool bFirstCallThisFrame )
{
...
void CDetailObjectSystem::RenderTranslucentDetailObjectsInLeaf( const DistanceFadeInfo_t &info, const Vector &viewOrigin, const Vector &viewForward, const Vector &viewRight, const Vector &viewUp, int nLeaf, const Vector *pVecClosestPoint )
{
	...
	if ( m_nSortedLeaf != nLeaf )
	{
		...
		// Sort detail sprites in each leaf independently; then render them
		m_nSpriteCount = SortSpritesBackToFront( nLeaf, viewOrigin, viewForward, info, m_pSortInfo );
...
void CDetailObjectSystem::BuildDetailObjectRenderLists( const Vector &vViewOrigin )
{
	...
	ctx.m_vViewOrigin = vViewOrigin;
 	ctx.m_BuildWorldListNumber = GetViewRenderInstance()->BuildWorldListsNumber();
