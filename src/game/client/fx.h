...
void FX_BulletPass( const Vector& start, const Vector& end );
...
void FX_BugBlood( const Vector &pos, const Vector &dir, const Vector &vWorldMins, const Vector &vWorldMaxs );
void FX_Blood( const Vector &pos, const Vector &dir, float r, float g, float b, float a );
void FX_CreateImpactDust( const Vector &origin, const Vector &normal );
...
void FX_Smoke( const Vector &origin, const QAngle &angles, float scale, int numParticles, const unsigned char *pColor = NULL, int iAlpha = -1 );
...
void FX_GaussTracer( const Vector& start, const Vector& end, int velocity, bool makeWhiz = true );
