...
void CLCD::Init( void )
{
	...
	if ( kv->LoadFromFile( g_pFullFileSystem, G15_RESOURCE_FILE, "MOD" ) )
	{
...
void CLCD::ParseIconMappings( KeyValues *kv )
{
	for ( KeyValues *icon = kv->GetFirstSubKey(); icon; icon = icon->GetNextKey() )
	{
		...
		char fullpath[ 512 ];
		g_pFullFileSystem->RelativePathToFullPath( icon->GetString(), "GAME", fullpath, sizeof( fullpath ) );
#ifdef WIN32
...
void CDescribeData::DescribeString( const string_t *instring, int count )
{
	for ( int i = 0; i < count; ++i )
	{
		if ( count == 1 )
		{
			Describe( " string (%s)\n", instring[ i ] != NULL_STRING ? STRING(instring[ i ])  : "" );
		}
		else
		{
			Describe( "[%i] string (%s)\n", i, instring[ i ] != NULL_STRING ? STRING(instring[ i ])  : "" );
		}
	}
}
