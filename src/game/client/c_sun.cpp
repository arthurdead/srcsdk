...
void C_Sun::OnDataChanged( DataUpdateType_t updateType )
{
	...
	const model_t* pModel = IsValidModelIndex( m_nMaterial ) ? modelinfo->GetModel( m_nMaterial ) : NULL;
	...
	pModel = IsValidModelIndex( m_nOverlayMaterial ) ? modelinfo->GetModel( m_nOverlayMaterial ) : NULL;
