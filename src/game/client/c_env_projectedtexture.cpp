...
void UTIL_ColorToLinearFloatColor( Vector &vecColor, color32 clr )
{
	float tmp[4];
	...
	vecColor.x = GammaToLinear( tmp[0] * ( 1.0f / 255.0f ) ) * tmp[3];
	vecColor.y = GammaToLinear( tmp[1] * ( 1.0f / 255.0f ) ) * tmp[3];
	vecColor.z = GammaToLinear( tmp[2] * ( 1.0f / 255.0f ) ) * tmp[3];
}
...
void C_EnvProjectedTexture::UpdateLight( bool bForceUpdate )
{
	...
	Vector linearFloatLightColor;
	UTIL_ColorToLinearFloatColor( linearFloatLightColor, m_LightColor );

	m_FlashlightState.m_fQuadraticAtten = 0.0;
	m_FlashlightState.m_fLinearAtten = 100;
	m_FlashlightState.m_fConstantAtten = 0.0f;
	m_FlashlightState.m_Color[0] = linearFloatLightColor.x;
	m_FlashlightState.m_Color[1] = linearFloatLightColor.y;
	m_FlashlightState.m_Color[2] = linearFloatLightColor.z;
