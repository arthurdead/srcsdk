...
const void *CFlexSceneFileManager::FindSceneFile( IHasLocalToGlobalFlexSettings *instance, const char *filename, bool allowBlockingIO )
{
	...
	void *buffer = NULL;
	int len = g_pFullFileSystem->ReadFileEx( VarArgs( "expressions/%s.vfe", szFilename ), "GAME", &buffer );
...
void C_BaseFlex::GetToolRecordingState( KeyValues *msg )
{
	...
	for (i = LocalFlexController_t(0); i < hdr->numflexcontrollers(); i++)
	{
		...
		s_pGlobalFlexWeight[pflex->localToGlobal] = m_flexWeight[i];
		// rescale
		s_pGlobalFlexWeight[pflex->localToGlobal] = s_pGlobalFlexWeight[pflex->localToGlobal] * (pflex->max - pflex->min) + pflex->min;
	}
...
bool C_BaseFlex::SetupGlobalWeights( const matrix3x4_t *pBoneToWorld, int nFlexWeightCount, float *pFlexWeights, float *pFlexDelayedWeights )
{
	...
	if (t > 0)
	{
		...
		if (t > 0.0f && t < 1.0f)
		{
			...
			s_pGlobalFlexWeight[m_iBlink] = clamp( s_pGlobalFlexWeight[m_iBlink] + t, 0.0f, 1.0f );
		}
	}
...
void C_BaseFlex::SetupLocalWeights( const matrix3x4_t *pBoneToWorld, int nFlexWeightCount, float *pFlexWeights, float *pFlexDelayedWeights )
{
	...
	if ( pFlexDelayedWeights )
	{
		...
		memcpy( m_CachedDelayedFlexWeights.Base(), m_flFlexDelayedWeight, nFlexWeightCount * sizeof(float) );
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
bool C_BaseFlex::StartSceneEvent( CSceneEventInfo *info, CChoreoScene *scene, CChoreoEvent *event, CChoreoActor *actor, C_BaseEntity *pTarget )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
bool C_BaseFlex::RequestStartSequenceSceneEvent( CSceneEventInfo *info, CChoreoScene *scene, CChoreoEvent *event, CChoreoActor *actor, C_BaseEntity *pTarget )
{
