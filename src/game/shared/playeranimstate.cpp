...
void CPlayerAnimState::Init( CSharedBasePlayer *pPlayer, const PlayerMovementData_t &movementData )
{
	...
	memcpy( &m_MovementData, &movementData, sizeof( PlayerMovementData_t ) );
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CPlayerAnimState::PlayFlinchGesture( Activity iActivity )
{
...
// Purpose:
//-----------------------------------------------------------------------------
bool CPlayerAnimState::InitGestureSlots( void )
{
...
// Purpose:
//-----------------------------------------------------------------------------
void CPlayerAnimState::ShutdownGestureSlots( void )
{
...
void CPlayerAnimState::ResetGestureSlot( gestureslotindex_t iGestureSlot )
{
	...
	if ( pGestureSlot )
	{
		...
		{
			pGestureSlot->m_pAnimLayer->SetOrder( MAX_ANIM_OVERLAYS );
#ifdef CLIENT_DLL
...
// Purpose: 
//-----------------------------------------------------------------------------
void CPlayerAnimState::ShowDebugInfo( void )
{
...
//-----------------------------------------------------------------------------
bool CPlayerAnimState::HandleJumping( Activity &idealActivity )
{
	if ( m_bJumping )
	{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CPlayerAnimState::HandleDucking( Activity &idealActivity )
{
	if ( GetBasePlayer()->GetFlags() & FL_DUCKING )
	{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CPlayerAnimState::HandleVehicle( Activity &idealActivity )
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CPlayerAnimState::HandleSwimming( Activity &idealActivity )
{
	if ( GetBasePlayer()->GetWaterLevel() >= WL_Waist )
	{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CPlayerAnimState::HandleDying( Activity &idealActivity )
{
	if ( m_bDying )
	{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CPlayerAnimState::HandleMoving( Activity &idealActivity )
{
...
// Output : Activity
//-----------------------------------------------------------------------------
Activity CPlayerAnimState::TranslateActivity( Activity actDesired )
{
...
// Output : float
//-----------------------------------------------------------------------------
float CPlayerAnimState::CalcMovementSpeed( bool *bIsMoving )
{
...
// Output : float
//-----------------------------------------------------------------------------
float CPlayerAnimState::GetInterpolatedGroundSpeed( void )
{
...
// Input  : *pStudioHdr - 
//-----------------------------------------------------------------------------
void CPlayerAnimState::ComputeSequences( CStudioHdr *pStudioHdr )
{
...
// Input  :  - 
//-----------------------------------------------------------------------------
void CPlayerAnimState::ComputeMainSequence()
{
...
// Input  :  - 
//-----------------------------------------------------------------------------
void CPlayerAnimState::UpdateInterpolators()
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CPlayerAnimState::ComputeFireSequence( void )
{
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CPlayerAnimState::UpdateGestureLayer( CStudioHdr *pStudioHdr, GestureSlot_t *pGesture )
{
	// Sanity check.
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CPlayerAnimState::SetupPoseParameters( CStudioHdr *pStudioHdr )
{
...
//-----------------------------------------------------------------------------
void CPlayerAnimState::DoMovementTest( CStudioHdr *pStudioHdr, float flX, float flY )
{
...
void CPlayerAnimState::DoMovementTest( CStudioHdr *pStudioHdr )
{
...
void CPlayerAnimState::GetMovementFlags( CStudioHdr *pStudioHdr )
{
...
// Input  : *pStudioHdr - 
//-----------------------------------------------------------------------------
void CPlayerAnimState::ComputePoseParam_MoveYaw( CStudioHdr *pStudioHdr )
{
...
//			&flCurrentYaw - 
//-----------------------------------------------------------------------------
void CPlayerAnimState::ConvergeYawAngles( float flGoalYaw, float flYawRate, float flDeltaTime, float &flCurrentYaw )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CPlayerAnimState::Release( void )
{
	delete this;
}
...
// Output : float
//-----------------------------------------------------------------------------
float CPlayerAnimState::GetOuterXYSpeed()
{
	Vector vel;
...
// Purpose:
//-----------------------------------------------------------------------------
void CPlayerAnimState::DebugShowAnimStateForPlayer()
{
...
//	Anim_StateLog( "-------------%s: frame %d -----------------\n", bIsServer ? "Server" : "Client", gpGlobals->framecount );
	...
	for ( int iAnim = 0; iAnim < GetBasePlayer()->GetNumAnimOverlays(); ++iAnim )
	{
		...
		if ( pLayer && ( pLayer->m_nOrder != MAX_ANIM_OVERLAYS ) )
		{
...
// Purpose:
//-----------------------------------------------------------------------------
void CPlayerAnimState::DebugShowEyeYaw( void )
{
#ifdef _NDEBUG
...
// Input  : activity - 
//-----------------------------------------------------------------------------
void CPlayerAnimState::DebugShowActivity( Activity activity )
{
#ifdef _DEBUG
...
// Input  : iStartLine - 
//-----------------------------------------------------------------------------
void CPlayerAnimState::DebugShowAnimState( int iStartLine )
{
	Vector vOuterVel;
	...
	for ( int i=0; i < GetBasePlayer()->GetNumAnimOverlays()-1; i++ )
	{
		...
		Anim_StatePrintf( iLine++, "%s, weight: %.2f, cycle: %.2f, aim (%d)", 
			pLayer->m_nOrder == MAX_ANIM_OVERLAYS ? "--" : GetSequenceName( GetBasePlayer()->GetModelPtr(), pLayer->m_nSequence ), 
			pLayer->m_nOrder == MAX_ANIM_OVERLAYS ? -1 :(float)pLayer->m_flWeight, 
			pLayer->m_nOrder == MAX_ANIM_OVERLAYS ? -1 :(float)pLayer->m_flCycle, 
			i
			);
	}
#endif
...
//-----------------------------------------------------------------------------
void CPlayerAnimState::OnNewModel( void )
{
	m_bPoseParameterInit = false;
	m_PoseParameterData.Init();
	ClearAnimationState();
