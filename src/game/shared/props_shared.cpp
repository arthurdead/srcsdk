...
const char *CPropData::GetRandomChunkModel( const char *pszBreakableSection, int iMaxSize )
{
	...
	int nBreakSecLen = strlen(pszBreakableSection);
	...
	for ( i = 0; i < iCount; i++ )
	{
		if ( !Q_strncmp( STRING(m_BreakableChunks[i].iszChunkType), pszBreakableSection, nBreakSecLen ) )
			break;
	}
	if ( i == iCount )
		return NULL;
...
CGameGibManager::CGameGibManager() : m_iCurrentMaxPieces(-1), m_iMaxPieces(-1), m_iMaxPiecesDX8(-1)
{
	g_pGibManager = this;
}
...
bool CPhysicsProp::GetPropDataAngles( const char *pKeyName, QAngle &vecAngles )
{
	...
	{
		KeyValues *pkvPropData = pModelKV->FindKey( "physgun_interactions" );
		if ( pkvPropData )
		{
