...
#define SUBINCH_PRECISION	3
	...
	static void RecvProxy_EntIndex( const CRecvProxyData *pData, void *pStruct, void *pOut )
	{
		...
		((CEffectData*)pStruct)->m_hEntity = (nEntIndex < 0) ? NULL_EHANDLE : ClientEntityList().EntIndexToHandle( nEntIndex );
	}
	...
	BEGIN_SEND_TABLE_NOBASE( CEffectData, DT_EffectData )
...
#ifdef HL2_DLL
		SendPropFloat( SENDINFO_NOCHECK_VECTORELEM( m_vOrigin[0] ), COORD_INTEGER_BITS+SUBINCH_PRECISION, SPROP_NONE, MIN_COORD_INTEGER, MAX_COORD_INTEGER ),
		SendPropFloat( SENDINFO_NOCHECK_VECTORELEM( m_vOrigin[1] ), COORD_INTEGER_BITS+SUBINCH_PRECISION, SPROP_NONE, MIN_COORD_INTEGER, MAX_COORD_INTEGER ),
		SendPropFloat( SENDINFO_NOCHECK_VECTORELEM( m_vOrigin[2] ), COORD_INTEGER_BITS+SUBINCH_PRECISION, SPROP_NONE, MIN_COORD_INTEGER, MAX_COORD_INTEGER ),
		SendPropFloat( SENDINFO_NOCHECK_VECTORELEM( m_vStart[0] ), COORD_INTEGER_BITS+SUBINCH_PRECISION, SPROP_NONE, MIN_COORD_INTEGER, MAX_COORD_INTEGER ),
		SendPropFloat( SENDINFO_NOCHECK_VECTORELEM( m_vStart[1] ), COORD_INTEGER_BITS+SUBINCH_PRECISION, SPROP_NONE, MIN_COORD_INTEGER, MAX_COORD_INTEGER ),
		SendPropFloat( SENDINFO_NOCHECK_VECTORELEM( m_vStart[2] ), COORD_INTEGER_BITS+SUBINCH_PRECISION, SPROP_NONE, MIN_COORD_INTEGER, MAX_COORD_INTEGER ),
#else
		...
		SendPropVector( SENDINFO_NOCHECK( m_CustomColors.m_vecColor1 ), 8, SPROP_NONE, 0, 1 ),
		SendPropVector( SENDINFO_NOCHECK( m_CustomColors.m_vecColor2 ), 8, SPROP_NONE, 0, 1 ),
