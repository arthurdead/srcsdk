...
void physicssound::PlayImpactSounds( soundlist_t &list )
{
	for ( int i = list.Count()-1; i >= 0; --i )
	{
		...
		if ( psurf->sounds.impactHard )
		{
			...
			if ( !CSharedBaseEntity::GetParametersForSound( pSound, params, NULL ) )
				break;
			...
			CSharedBaseEntity::EmitSound( filter, 0 /*sound.entityIndex*/, ep );
		}
	}
	list.RemoveAll();
}
...
void physicssound::AddBreakSound( CUtlVector<breaksound_t> &list, const Vector &origin, unsigned short surfaceProps )
{
...
void physicssound::PlayBreakSounds( CUtlVector<breaksound_t> &list )
{
	for ( int i = list.Count()-1; i >= 0; --i )
	{
		breaksound_t &sound = list.Element(i);
