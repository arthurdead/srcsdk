...
static AI_StandoffParams_t AI_DEFAULT_STANDOFF_PARAMS = {AIHCR_MOVE_ON_COVER, true, true, 1.5, 2.5, 1, 3, 25, false, 0.0};
...
int CAI_StandoffBehavior::SelectScheduleUpdateWeapon( void )
{
	...
	{
		// if hurt:
		int iPercent = random_valve->RandomInt(0,99);
...
int CAI_StandoffBehavior::SelectScheduleAttack( void )
{
	if ( GetPosture() == AIP_PEEKING || GetPosture() == AIP_STANDING )
	{
			  ...
			  HasCondition( COND_TOO_FAR_TO_ATTACK ) )
		{
			...
			{
				if ( !HasCondition( COND_ENEMY_OCCLUDED ) || random_valve->RandomInt(0,99) < 50 )
	...
	switch ( pTask->iTask )
	{
		...
		case TASK_FIND_COVER_FROM_ENEMY:
		{
			...
			if ( !m_params.fStayAtCover && (!m_TimeForceCover.Expired() || m_RandomCoverChangeTimer.Expired()) )
			{
				...
				{
					if( random_valve->RandomInt(1, 100) <= 50 )
					{
