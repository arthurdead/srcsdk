...
void CAI_Expresser::TestAllResponses()
{
	...
	if ( pResponseSystem )
	{
		...
		for ( int i = 0; i < responses.Count(); i++ )
		{
			...
			Msg( "Response: %s\n", response );
			AIConcept_t conc;
			SpeakDispatchResponse( conc, &responses[i], NULL, NULL );
		}
	}
}
...
bool CAI_Expresser::FindResponse( AI_Response &outResponse, AIConcept_t &conc, AI_CriteriaSet *criteria )
{
	...
	{
		// AssertMsg2( false, "RR: %s got empty but valid response for %s", GetOuter()->GetDebugName(), conc.GetStringConcept() );
		return false;
	}
	else
	{
...
bool CAI_Expresser::FindResponse( AI_Response &outResponse, AIConcept_t &conc, const char *modifiers )
{
	AI_CriteriaSet set;
	// Always include the concept name
	...
	return FindResponse(outResponse, conc, &set);
}
...
bool CAI_Expresser::FireEntIOFromResponse( char *response, CBaseEntity *pInitiator )
{
	...
	pszEntname = strtok_r( response, " ", &strtokContext );
	if ( !pszEntname )
	{
	...
	pszInput = strtok_r( NULL, " ", &strtokContext );
	if ( !pszInput )
	{
		Warning( "Response was entityio but had bad value %s\n", response );
		return false;
	...
	pszParam =  strtok_r( NULL, " ", &strtokContext );
...
void CAI_Expresser::AllowMultipleScenes()
{
	m_bAllowMultipleScenes = true;
}

void CAI_Expresser::DisallowMultipleScenes()
{
	m_bAllowMultipleScenes = false;
}
