...
//=========================================================
//=========================================================
class CNewNPC : public CAI_BaseHumanoid
{
	DECLARE_CLASS( CNewNPC, CAI_BaseHumanoid );
...
IMPLEMENT_CUSTOM_AI( npc_newnpc,CNewNPC );
...
void CNewNPC::Spawn( void )
{
	...
	m_FollowBehavior.SetFollowTarget(player, true);

	NPCInit();

	m_debugOverlays |= OVERLAY_NPC_TRIANGULATE_BIT|OVERLAY_NPC_NEAREST_BIT|OVERLAY_NPC_ROUTE_BIT;

	CBaseEntity::m_bInDebugSelect = true;
	CBaseEntity::m_nDebugPlayer = player->entindex();

	int seq = SelectWeightedSequence(ACT_RUN);
	SetActivityAndSequence(ACT_RUN, seq, ACT_RUN, ACT_RUN);

	SetPoseParameter("move_x", 1.0f);
}

void CNewNPC::DrawDebugGeometryOverlays(void)
{
	 BaseClass::DrawDebugGeometryOverlays();

	 m_FollowBehavior.DrawDebugGeometryOverlays();
}

int	CNewNPC::DrawDebugTextOverlays(void)
{
	int ret = BaseClass::DrawDebugTextOverlays();

	m_FollowBehavior.DrawDebugTextOverlays(0);

	return ret;
}

bool CNewNPC::CreateBehaviors()
{
...
// Purpose: 
//-----------------------------------------------------------------------------
int CNewNPC::SelectSchedule( void )
{
