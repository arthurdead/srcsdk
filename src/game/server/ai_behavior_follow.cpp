...
{ 
	if ( pLeader == m_hFollowTarget.Get() )
		return;
...
bool CAI_FollowBehavior::IsFollowGoalInRange( float tolerance, float zTolerance, int flags )
{
	...
	{
		if ( !GetOuter()->GetSenses()->DidSeeEntity( m_hFollowTarget.Get() ) )
			return false;
	}
...
void CAI_FollowBehavior::Precache()
{
	...
	{
		// Post load fixup
		if ( !g_AIFollowManager.AddFollower( m_hFollowTarget.Get(), GetOuter(), m_params.formation, &m_hFollowManagerInfo ) )
		{
			m_hFollowTarget = NULL;
		}
	}
...
int CAI_FollowBehavior::SelectFailSchedule( int failedSchedule, int failedTask, AI_TaskFailureCode_t taskFailCode )
{
	if ( failedTask == TASK_MOVE_TO_FOLLOW_POSITION || failedTask == TASK_GET_PATH_TO_FOLLOW_POSITION )
	{
		if ( m_hFollowTarget )
		{
			m_TargetMonitor.SetMark( m_hFollowTarget.Get(), m_FollowNavGoal.targetMoveTolerance * 0.5 );	
			m_FollowDelay.Start();
			NoteFailedFollow();
		}
	}
...
int CAI_FollowBehavior::TranslateSchedule( int scheduleType )
{
	switch( scheduleType )
	{
		...
		case SCHED_TARGET_FACE:
		{
			...
			if ( !m_TargetMonitor.IsMarkSet() )
				m_TargetMonitor.SetMark( m_hFollowTarget.Get(), m_FollowNavGoal.targetMoveTolerance );
...
void CAI_FollowBehavior::GetFollowTargetViewLoc( Vector *pResult )
{
	if ( !dynamic_cast<CPointEntity *>(m_hFollowTarget.Get()) )
	{
		...
		AI_TraceLOS( vecStart, vecStart + vecDir * 8192, m_hFollowTarget.Get(), &tr );
...
bool CAI_FollowBehavior::ValidateFaceTarget( Vector *pFaceTarget )
{
	...
	trace_t tr;
	AI_TraceLine( GetOuter()->EyePosition(), testPoint, MASK_BLOCKLOS, m_hFollowTarget.Get(), COLLISION_GROUP_NONE, &tr );
			...
			case 2:
				{
					...
					{
						goal.pTarget = m_hFollowTarget.Get();
					}
					else
					{
...
bool CAI_FollowBehavior::IsValidCover( const Vector &vLocation )
{
...
bool CAI_FollowBehavior::IsValidShootPosition( const Vector &vLocation )
{
...
bool CAI_FollowManager::CalcFollowPosition( AI_FollowManagerInfoHandle_t& hInfo, AI_FollowNavInfo_t *pNavInfo )
{
	if ( hInfo.m_pGroup && hInfo.m_hFollower )
	{
		...
		CBaseEntity *pTarget = pGroup->hFollowTarget.Get();
...
bool CAI_FollowManager::RedistributeSlots( AI_FollowGroup_t *pGroup )
{
	...
	while ( ( bestSlot = FindBestSlot( pGroup ) ) != -1 && ((int)movedFollowers.Count() < pGroup->followers.Count()) )
	{
		...
		while ( h != pGroup->followers.InvalidIndex() )
		{
			...
			if ( movedFollowers.Find( p->hFollower.Get() ) == movedFollowers.InvalidIndex() && 
		...
		movedFollowers.Insert( pBest->hFollower.Get() );
		result = true;
	}
	return result;
}
