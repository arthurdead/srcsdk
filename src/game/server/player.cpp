...
void CBasePlayer::Event_Killed( const CTakeDamageInfo &info )
{
	...
	g_pGameRules->PlayerKilled( this, newinfo );
...
bool CBasePlayer::HandleCommand_JoinTeam(int team)
{
...
void CBasePlayer::GetAutoaimVector( autoaim_params_t &params )
{
	...
	if( IsInAVehicle() && g_pGameRules->GetAutoAimMode() == AUTOAIM_EXTRA )
	{
		m_vecAutoAim = angles;
		...
		for ( int i = 0; i < count; i++ )
		{
			...
			if( !(pEntity->GetFlags() & FL_FLY) )
			{
				...
				if( GetActiveWeapon() != NULL && dot < GetActiveWeapon()->GetMaxAutoAimDeflection() )
				{
					...
					if( eyeAngles.x < 30.0f || eyeAngles.x > 90.0f || g_pGameRules->GetAutoAimMode() != AUTOAIM_EXTRA )
					{
						continue;
					}
				}
