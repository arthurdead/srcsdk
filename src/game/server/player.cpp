...
void CBasePlayer::PlayerDeathThink(void)
{
	...
	if (IsValidModelIndex(GetModelIndex()) && (!IsSequenceFinished()) && (m_lifeState == LIFE_DYING))
	{
		StudioFrameAdvance( );
...
// -------------------------------------------------------------------------------- //
	...
	BEGIN_SEND_TABLE_NOBASE( CBasePlayer, DT_LocalPlayerExclusive )
		...
		SendPropFloat		( SENDINFO_VECTORELEM(m_vecVelocity, 2), 32, SPROP_NOSCALE|SPROP_CHANGES_OFTEN, 0, HIGH_DEFAULT, SendProxy_Float, SENDPROP_PLAYER_VELOCITY_Z_PRIORITY ),
