...
void CAI_RappelBehavior::StartTask( const Task_t *pTask )
{
	...
	case TASK_HIT_GROUND:
		...
		if( GetOuter()->GetGroundEntity() != NULL && GetOuter()->GetGroundEntity()->IsNPC() && GetOuter()->GetGroundEntity()->GetClassnameStr() == GetOuter()->GetClassnameStr() )
		{
...
AI_BEGIN_CUSTOM_SCHEDULE_PROVIDER( CAI_RappelBehavior )
	...
	DECLARE_TASK( TASK_RAPPEL, TaskParamCheck_t() )
	DECLARE_TASK( TASK_HIT_GROUND, TaskParamCheck_t() )
