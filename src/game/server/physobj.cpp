				...
				if ( HasSpawnFlags( SF_PHYSEXPLOSION_PUSH_PLAYER ) )
				{
					if ( pEntity->IsPlayer() )
					{
						...
						if ( HasSpawnFlags( SF_PHYSEXPLOSION_DISORIENT_PLAYER ) )
						{
							...
							vecDeltaAngles.x = random_valve->RandomInt( -30, 30 );
							vecDeltaAngles.y = random_valve->RandomInt( -30, 30 );
...
static CBaseEntity *CreateSimplePhysicsObject( CBaseEntity *pEntity, bool createAsleep, bool createAsDebris )
{
	...
	if(DispatchSpawn(pPhysEntity) < 0) {
		UTIL_Remove(pPhysEntity);
		return NULL;
	}
	...
	for ( int i = 0; i < count; i++ )
	{
		...
		if ( pPhys )
		{
			...
			if (pEntity->GetBaseAnimating() /*&& pPhys->GetBaseAnimating()*/)
			{
				...
				pPhysAnimating->SetSkin( pEntityAnimating->GetSkin() );
				pPhysAnimating->SetBody( pEntityAnimating->GetBody() );
