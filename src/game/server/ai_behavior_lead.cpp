...
void CAI_LeadBehavior::StartTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
		...
		case TASK_STOP_MOVING:
		{
			...
			if ( IsCurSchedule( SCHED_LEAD_PAUSE, false ) && pTask->data[0].AsBool() )
			{
...
AI_BEGIN_CUSTOM_SCHEDULE_PROVIDER( CAI_LeadBehavior )
	...
	// Lead
	//
	DECLARE_TASK( TASK_GET_PATH_TO_LEAD_GOAL, TaskParamCheck_t() )
	DECLARE_TASK( TASK_STOP_LEADING , TaskParamCheck_t())
	DECLARE_TASK( TASK_LEAD_ARRIVE , TaskParamCheck_t())
	DECLARE_TASK( TASK_LEAD_SUCCEED , TaskParamCheck_t())
	DECLARE_TASK( TASK_LEAD_FACE_GOAL , TaskParamCheck_t())
	DECLARE_TASK( TASK_LEAD_GET_PATH_TO_WAITPOINT , TaskParamCheck_t())
	DECLARE_TASK( TASK_LEAD_WAVE_TO_PLAYER , TaskParamCheck_t())
	DECLARE_TASK( TASK_LEAD_PLAYER_NEEDS_WEAPON , TaskParamCheck_t())
	DECLARE_TASK( TASK_LEAD_MOVE_TO_RANGE , TaskParamCheck_t())
	DECLARE_TASK( TASK_LEAD_SPEAK_START , TaskParamCheck_t())
	DECLARE_TASK( TASK_LEAD_RETRIEVE_WAIT , TaskParamCheck_t())
	DECLARE_TASK( TASK_LEAD_WALK_PATH , TaskParamCheck_t())

	DEFINE_SCHEDULE_FILE( SCHED_LEAD_RETRIEVE)

	//-------------------------------------

	DEFINE_SCHEDULE_FILE( SCHED_LEAD_SPEAK_THEN_RETRIEVE_PLAYER )

	//-------------------------------------

	DEFINE_SCHEDULE_FILE( SCHED_LEAD_RETRIEVE_WAIT )

	//---------------------------------

	DEFINE_SCHEDULE_FILE( SCHED_LEAD_PLAYER)

	//---------------------------------

	DEFINE_SCHEDULE_FILE( SCHED_LEAD_AWAIT_SUCCESS)

	//---------------------------------

	DEFINE_SCHEDULE_FILE( SCHED_LEAD_SUCCEED)

	DEFINE_SCHEDULE_FILE( SCHED_LEAD_PAUSE)

	DEFINE_SCHEDULE_FILE(SCHED_LEAD_PAUSE_COMBAT)

	//---------------------------------

	DEFINE_SCHEDULE_FILE( SCHED_LEAD_WAITFORPLAYER)

	//---------------------------------

	DEFINE_SCHEDULE_FILE( SCHED_LEAD_WAITFORPLAYERIDLE)

	//---------------------------------

	DEFINE_SCHEDULE_FILE( SCHED_LEAD_PLAYERNEEDSWEAPON)

	//---------------------------------

	DEFINE_SCHEDULE_FILE( SCHED_LEAD_SPEAK_START)

	//---------------------------------

	DEFINE_SCHEDULE_FILE( SCHED_LEAD_SPEAK_THEN_LEAD_PLAYER)
