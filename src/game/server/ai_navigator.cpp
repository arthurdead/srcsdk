...
COMPILE_TIME_ASSERT(ARRAYSIZE(g_ppszGoalTypes) == GOALTYPE_INVALID);
...
bool CAI_Navigator::FindPath( const AI_NavGoal_t &goal, unsigned flags )
{
	...
	else
	{
		...
		else if ( goal.arrivalSequence != INVALID_SEQUENCE )
		{
...
bool CAI_Navigator::SetRandomGoal( const Vector &from, float minPathLength, const Vector &dir )
{
	...
	AI_Waypoint_t* pRoute = GetPathfinder()->FindShortRandomPath( from, minPathLength, dir );
...
bool CAI_Navigator::SetDirectGoal( const Vector &goalPos, Navigation_t navType )
{
	...
	GetPath()->SetWaypoints( new AI_Waypoint_t( goalPos, 0, navType, bits_WP_TO_GOAL ) );
	GetPath()->SetGoalTolerance( GetOuter()->GetDefaultNavGoalTolerance() );
...
static bool SetWanderGoalByRandomVector(CAI_Navigator *pNav, float minRadius, float maxRadius, int numTries)
{
	while (--numTries >= 0)
	{
		float dist = random_valve->RandomFloat( minRadius, maxRadius );
		Vector dir = UTIL_YawToVector( random_valve->RandomFloat( 0, 359.99 ) );
...
bool CAI_Navigator::PrependLocalAvoidance( float distObstacle, const AIMoveTrace_t &directTrace )
{
	...
	if ( distObstacle < GetHullWidth() * 0.5 )
	{
		...
		AI_MoveLimitFlags_t flags = ( GetNavType() == NAV_GROUND ) ? AIMLF_2D : AIMLF_DEFAULT;
													 ...
													 flags, &backawayTrace ) )
		{
			...
			pAvoidanceRoute = new AI_Waypoint_t( vStart, 0, GetNavType(), bits_WP_TO_DETOUR );
		}
	}
...
void CAI_Navigator::PrependWaypoint( const Vector &newPoint, Navigation_t navType, WaypointFlags_t waypointFlags )
{
...
WaypointFlags_t CAI_Navigator::GetCurWaypointFlags() const
{
...
AI_NavGoalFlags_t	CAI_Navigator::GetGoalFlags() const
{
...
{
	return ( GetPath()->CurWaypointFlags() & bits_WP_PRECISE_MOVEMENT ) != bits_WP_NO_FLAGS;
}
...
AIMoveResult_t CAI_Navigator::MoveCrawl()
{
	...
	sequence_t			nPreMoveSequence	= GetOuter()->GetSequence(); // this is an unfortunate necessity to ensure setting back the activity picks the right one if it had been sticky
...
void CAI_Navigator::AdvancePath()
{
	...
	AI_Waypoint_t *pCurWaypoint = GetPath()->GetCurWaypoint();
	bool bPassingPathcorner = ( ( pCurWaypoint->Flags() & bits_WP_TO_PATHCORNER ) != bits_WP_NO_FLAGS );
	...
	if ( bPassingPathcorner )
	{
		...
		CBaseEntity *pEntity = pCurWaypoint->hPathCorner.Get();
		if ( pEntity )
		{
	...
	if ( bPassingPathcorner )
	{
		...
		if ( pCurWaypoint )
		{
			...
			SetGoalEnt( pCurWaypoint->hPathCorner.Get() );
			DoFindPathToPathcorner( pCurWaypoint->hPathCorner.Get() );
		}
	}
}
...
{
	AI_MoveLimitFlags_t flags = AIMLF_QUICK_REJECT;
...
void CAI_Navigator::SimplifyPathInsertSimplification( AI_Waypoint_t *pSegmentStart, const Vector &point )
{
	if ( point != pSegmentStart->GetPos() )
	{
		...
		AI_Waypoint_t *pNewWaypoint = new AI_Waypoint_t( point, 0, pSegmentStart->NavType(), bits_WP_NO_FLAGS );
											 ...
											 float distRemaining, bool skipTest, bool passedDetour, int *pTestCount )
{
	...
	if ( !passedDetour )
		passedDetour = ( ( pCurWaypoint->Flags() & bits_WP_TO_DETOUR) != bits_WP_NO_FLAGS );
...
bool CAI_Navigator::SimplifyPathForward( float maxDist )
{
	...
	if ( ShouldAttemptSimplifyTo( pNextWaypoint->GetPos() ) && 
		 ...
		 ShouldSimplifyTo( ( ( pCurWaypoint->Flags() & bits_WP_TO_DETOUR ) != bits_WP_NO_FLAGS ), pNextWaypoint->GetPos() ) ) // @TODO (toml 04-25-03): need to handle this better. this is here because forward scan may look out so far that a close obvious solution is skipped (due to test limiting)
	{
		delete pCurWaypoint;
...
bool CAI_Navigator::SimplifyPathBacktrack()
{
		...
		(pCurWaypoint->NavType() == NAV_GROUND) )	
	{
		...
		if (firstProj>0.5*firstDist) 
		{
			...
			RouteBuildFlags_e buildFlags = bits_BUILD_NO_FLAGS;
			...
			AI_Waypoint_t *route1 = GetPathfinder()->BuildLocalRoute(GetLocalOrigin(), targetPos, GetPath()->GetTarget(), bits_WP_TO_DETOUR, buildFlags, goalTolerance);
			if (!route1) 
				return false;

			// Make sure the new point gets me to the target location
			AI_Waypoint_t *route2 = GetPathfinder()->BuildLocalRoute(targetPos, pNextWaypoint->GetPos(), GetPath()->GetTarget(), bits_WP_TO_DETOUR, buildFlags, goalTolerance);
			if (!route2) 
			{
				DeleteAll(route1);
				return false;
...
AI_NavPathProgress_t CAI_Navigator::ProgressFlyPath( const AI_ProgressFlyPathParams_t &params )
{
	if ( IsGoalActive() )
	{
		...
		{
			bool bIsStrictWaypoint = ( (GetPath()->CurWaypointFlags() & (bits_WP_TO_PATHCORNER|bits_WP_DONT_SIMPLIFY) ) != bits_WP_NO_FLAGS );
...
void CAI_Navigator::SimplifyFlyPath( ContentsFlags_t collisionMask, const CBaseEntity *pTarget, 
...
bool CAI_Navigator::SimplifyFlyPath(  const AI_ProgressFlyPathParams_t &params )
{
	...
	bool bIsStrictWaypoint = ( !params.bTrySimplify || ( (GetPath()->CurWaypointFlags() & (bits_WP_TO_PATHCORNER|bits_WP_DONT_SIMPLIFY) ) != bits_WP_NO_FLAGS ) );
...
// Output :
//-----------------------------------------------------------------------------
float CAI_Navigator::MovementCost( Capability_t moveType, Vector &vecStart, Vector &vecEnd )
{
...
// Output :
//-----------------------------------------------------------------------------
bool CAI_Navigator::CanFitAtPosition( const Vector &vStartPos, ContentsFlags_t collisionMask, bool bIgnoreTransients, bool bAllowPlayerAvoid  )
{
...
bool CAI_Navigator::DoFindPathToPos(void)
{
	...
	RouteBuildFlags_e nBuildFlags = bits_BUILD_NO_FLAGS;
	...
	AI_Waypoint_t *pFirstWaypoint = pPathfinder->BuildRoute( origin, actualGoalPos, pTarget, tolerance, GetNavType(), m_bLocalSucceedOnWithinTolerance, nBuildFlags );
...
// circular paths, etc.
	...
	if ( pPathCorner != NULL )
	{
		...
		if ( ( GetGoalFlags() & AIN_NO_PATHCORNER_PATHFINDING ) || m_bNoPathcornerPathfinds )
		{
			...
			AI_Waypoint_t *pRoute = new AI_Waypoint_t( pPathCorner->GetLocalOrigin(), 0, GetNavType(), bits_WP_TO_PATHCORNER );
			...
			if ( pPathCorner )
			{
				pLast = new AI_Waypoint_t( pPathCorner->GetLocalOrigin(), 0, GetNavType(), bits_WP_TO_PATHCORNER );
		...
		else
		{
			...
			if ( ( returnCode = DoFindPathToPos() ) != false )
			{
				...
				if ( pPathCorner )
				{
					...
					AI_Waypoint_t *curWaypoint = new AI_Waypoint_t( pPathCorner->GetLocalOrigin(), 0, GetNavType(), (bits_WP_TO_PATHCORNER | bits_WP_TO_GOAL) );
		...
		if ( bMustCompleteCurrent || distRemaining > 0.1 )
		{
			...
			while ( distRemaining > 0.01  && pNextPoint )
			{
				...
				if ( distToNext <= distRemaining + 0.01 )
				{
					...
					pNewWaypoint->ModifyFlags( bits_WP_TO_GOAL | bits_WP_TO_POLYREF, false );
					pNewWaypoint->polyRef = 0;
...
//					NDebugOverlay::Cross3D( vPosPrev, 16, 255, 255, 0, false, 10.0f );
				...
				else
				{
					...
					{
						pNewWaypoint = new AI_Waypoint_t( trace.vEndPosition, yaw, pNextPoint->NavType(), bits_WP_TO_GOAL);
...
static Color GetRouteColor(Navigation_t navType, WaypointFlags_t waypointFlags)
{
	if (navType == NAV_JUMP)
	{
	...
	if (waypointFlags & bits_WP_TO_POLYREF)
	{
...
void CAI_Navigator::DrawDebugRouteOverlay(void)
{
	...
	while (waypoint) 
	{
		...
		if(waypoint->Flags() & bits_WP_TO_POLYREF) {
			V_strcat_safe(szInfo, "PolyRef|");
		}
