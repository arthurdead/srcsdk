...
ConVar mapbase_ally_flinching("ai_ally_flinching", "1", FCVAR_ARCHIVE, "Enables/disables the new flinching animations.");
//-----------------------------------------------------------------------------
...
bool CAI_PlayerAlly::RespondedTo( const char *ResponseConcept, bool bForce, bool bCancelScene )
{
	...
	if ( SpeakIfAllowed( ResponseConcept, static_cast<AI_CriteriaSet *>(NULL), true ) )
		return true;
...
AI_BEGIN_CUSTOM_NPC(talk_monster_base,CAI_PlayerAlly)
	...
	DECLARE_TASK(TASK_TALKER_SPEAK_PENDING, TaskParamCheck_t())
